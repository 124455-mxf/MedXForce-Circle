...
export const circleSectionPanelClass = cn(
  'bg-[#F8FAFC] rounded-[32px] border border-slate-100 shadow-sm flex flex-col flex-1 min-h-0 min-w-0 overflow-hidden',
  `${short}rounded-2xl`,
);
...
export const circleHorizontalScrollClass = cn(
  ...
  '[&_button]:touch-action-[pan-x] [&_a]:touch-action-[pan-x]',
  '[scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden',
  '[-webkit-overflow-scrolling:touch]',
);

export const circleHorizontalScrollInnerClass = 'inline-flex flex-nowrap items-center w-max';
...
export const circleWorkTabHeaderClass = (compact: boolean): string => {
  return cn(
    'shrink-0 min-w-0 w-full border-b border-slate-100 bg-white/80',
