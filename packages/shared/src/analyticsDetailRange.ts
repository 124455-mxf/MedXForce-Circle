...
export type AnalyticsDetailRangeId = '7' | '30' | '90' | '180' | 'all';
...
export function parseAnalyticsDetailRangeId(raw: unknown): AnalyticsDetailRangeId {
  if (raw === '7' || raw === '30' || raw === '90' || raw === '180' || raw === 'all') return raw;
  return '30';
}

export function analyticsDetailRangeIdFromPeriodDays(days: 7 | 30): AnalyticsDetailRangeId {
  return days === 7 ? '7' : '30';
}
...
export function analyticsDetailRangeGrain(rangeId: AnalyticsDetailRangeId): AnalyticsDetailChartGrain {
  if (rangeId === '7' || rangeId === '30') return 'day';
